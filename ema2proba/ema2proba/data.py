...
def extract_labels(time_vec, seg_file):
    ...
    label_array = relabel(time_vec, labels, insts)
    
    return label_array, insts, labels
    # phon_max = insts[:, -1]
    # count = len(labels)
...
def relabel(time_vector, seg_lbls, seg_bounds):
    return [labelling(t, seg_lbls, seg_bounds) for t in time_vector]

def labelling(t, seg_lbls, seg_bounds):
    return [seg_lbls[i] for i in range(len(seg_lbls)) if 
            t >= seg_bounds[i, 0]  and t <= seg_bounds[i, 1]][0]
