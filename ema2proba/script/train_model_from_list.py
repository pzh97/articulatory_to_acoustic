...
parser.add_argument("data_file", metavar="data_file", type=str,
                    help="""Path to file containing the list of data 
                    files for training""")
...
if not path.isfile(data_file):
    raise ValueError("List file " + data_file + " does not exist!")
else:
    print("Writing feature files in", path.abspath(path.join(".", "features")), 
          flush=True)
    if path.isdir(path.abspath(path.join(".", "features"))):
        print(path.abspath(path.join(".", "features")) + 
             " already exists. Folder will be deleted", flush=True)
        shutil.rmtree(path.abspath(path.join(".", "features")))
    ema2proba.write_features_from_list(data_file, path.join(".", "features"))
    dirs = listdir(path.join(".", "features"))
    nb_spk = len(dirs)
    print("Features files successfully written", flush=True)
    print("Number of speakers:", nb_spk, flush=True)
    ...
    speaker_scalers = []
    for d in dirs:
        print("Building feature matrix for speaker", d)
        feature_dir = path.join(path.join(".", "features"), d)
        ...
        if voicing_merging:
            ...
            y = ema2proba.merge_phoneme_strings(phoneme_merging, y,
                                                       verbosity=verbosity)
            voicing_col = [15*n + 14 for n in range(2*delta+1)]
            X = np.delete(X, voicing_col, axis=1)
            isVoicing = False
            
        print("Speaker normalization...", flush=True)
        X, scaler = ema2proba.normalization(X, norm=normalization)
        speaker_scalers.append(scaler)
        features.append(X)
        categories.append(y)
        [phonemes.append(p) for p in np.unique(y)] 

X = features[0]
y = categories[0]
phonemes = np.unique(phonemes).tolist()
for f, c in zip(features[1:], categories[1:]):
    X = np.vstack((X, f))
    y += c
