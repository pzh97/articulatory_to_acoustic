...
2.4 File Naming Convention
			...
			msak0_???.lab ASCII label file
			msak0_???.mlf ASCII HTK master label file
...
2.5 File Format
	...
	Label *.lab
		ASCII format Start_time Stop_time label where Start_time and 
		Stop_time are expressesd in seconds as floats and labels are 
		a variable length string. The times and labels are separated 
		by a single white space and each segment occurs on a new line.
...
2.7 Information for Windows users:
	...
	EMAtools works under the Windows version 5.3 or higher of MATLAB
        NB. When using WinZip to unbundle the data '*.tar.gz' files you will
        be prompted with a filename like 'fsew0_v1.1_tar.tar' Change this to 'fsew0_v1.1.tar'.
        Also you must ensure that the automatic ascii conversion option is
        switched off otherwise the ascii headers of the data files will be corrupted.
